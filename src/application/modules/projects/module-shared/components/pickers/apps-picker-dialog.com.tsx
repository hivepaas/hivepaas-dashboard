...
interface Props {
    ...
    projectID: string;
    excludeAppId?: string;
    onSelect: (app: AppPublic) => void;
}

export function AppsPickerDialog({ open, onOpenChange, projectID, excludeAppId, onSelect }: Props) {
    ...
    const filteredRows = useMemo(() => {
        ...
        return rows.filter(row => row.id !== excludeAppId);
    }, [excludeAppId, rows]);
                ...
                <DialogBody className="flex min-h-0 flex-col gap-5">
                    ...
                    <div className="min-h-0 overflow-auto rounded-md border">
                        <Table>
                            ...
                            <TableBody>
                                {query.isFetching && filteredRows.length === 0 ? (
                                    <TableRow>
                                        <TableCell
                                            colSpan={2}
