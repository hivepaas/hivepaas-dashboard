...
import { useAppPeriodicJobsApi } from "~/projects/api";
import type {
...
function useFindManyPaginated(request: FindManyPaginatedReq, options: FindManyPaginatedOptions = {}) {
    const { queries } = useAppPeriodicJobsApi();

    return useQuery({
        queryKey: [QK["projects.apps.periodic-jobs.$.find-many-paginated"], request],
...
function useFindOneById(request: FindOneByIdReq, options: FindOneByIdOptions = {}) {
    ...
    return useQuery({
        queryKey: [QK["projects.apps.periodic-jobs.$.find-one-by-id"], request],
