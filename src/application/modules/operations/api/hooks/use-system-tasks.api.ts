        ...
        const queries = useMemo(
            () => ({
                findManyPaginated: async (data: SystemTasks_FindManyPaginated_Req["data"], signal?: AbortSignal) => {
                    const result = await api.operations.tasks.findManyPaginated({ data }, signal);
                ...
                findOneById: async (data: SystemTasks_FindOneById_Req["data"], signal?: AbortSignal) => {
                    const result = await api.operations.tasks.findOneById({ data }, signal);
                ...
                findTypes: async (data: SystemTasks_FindTypes_Req["data"] = {}, signal?: AbortSignal) => {
                    const result = await api.operations.tasks.findTypes({ data }, signal);
        ...
        const mutations = useMemo(
            () => ({
                cancel: async (data: SystemTasks_Cancel_Req["data"]) => {
                    const result = await api.operations.tasks.cancel({ data });
