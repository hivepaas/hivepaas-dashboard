...
export const CreateOrEditNotificationTargetFormSchema = z
    .object({
        ...
        slackUseDefault: z.boolean(),
        slackWebhookId: z.string().trim(),
        discordEnabled: z.boolean(),
        discordUseDefault: z.boolean(),
    ...
    .superRefine((values, ctx) => {
        if (values.emailEnabled && !values.emailUseDefault && !values.senderEmailAccountId) {
            ctx.addIssue({
        ...
        if (values.slackEnabled && !values.slackUseDefault && !values.slackWebhookId) {
            ctx.addIssue({
                code: z.ZodIssueCode.custom,
                path: ["slackWebhookId"],
        ...
        if (values.discordEnabled && !values.discordUseDefault && !values.discordWebhookId) {
            ctx.addIssue({
                code: z.ZodIssueCode.custom,
        ...
        if (values.telegramEnabled && !values.telegramUseDefault && !values.telegramSettingId) {
            ctx.addIssue({
                code: z.ZodIssueCode.custom,
